...
def get_station_data(lookback_minutes=30):
    request_string = "https://api.synopticdata.com/v2/stations/timeseries?token=a63457b0f00743f1b593f78cb88b1fb0&recent={lookback_minutes}&stid=FPS&state=ut&units=english&obtimezone=LOCAL".format(
        token=config.token, lookback_minutes=lookback_minutes
    )
...
def check_wind(station_data):
    ...
    wind_top_value = 16
    wind_speed_is_acceptable = (
        (
            (last_3_wind_speeds > wind_bottom_value)
            & (last_3_wind_speeds < wind_top_value)
        )
        .all()
        .iloc[0]
    )
    ...
    wind_dir_is_acceptable = (
        (
            (last_3_wind_directions > bottom_wind_dir_value)
            & (last_3_wind_directions < top_win_dir_value)
        )
        .all()
        .iloc[0]
    )
...
def check_daytime():
    ...
    approx_sunrise = (s["sunrise"] - datetime.timedelta(minutes=10)).replace(
        tzinfo=pytz.timezone("America/Denver")
    )
    approx_sunset = (s["sunset"] - datetime.timedelta(minutes=30)).replace(
        tzinfo=pytz.timezone("America/Denver")
    )
...
def check_midday():
    ...
    s = sun(loc.observer, date=current_time, tzinfo="US/Mountain")
    two_h_after_sunrise = (s["sunrise"] + datetime.timedelta(hours=2)).replace(
        tzinfo=pytz.timezone("America/Denver")
    )
    three_h_before_sunset = (s["sunset"] - datetime.timedelta(hours=3)).replace(
        tzinfo=pytz.timezone("America/Denver")
    )
...
def check_all_conditions(station_data, winter):
    ...
    if not winter:
        ...
        midday = False
    all_conditions_are_right = (
        wind_is_acceptable
        and not is_raining
        and not strong_gusts
        and daytime
        and not midday
    )
